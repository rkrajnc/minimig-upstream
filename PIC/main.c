...
const char version[] = {"$VER:PYQ090405"};
...
void main(void)
{
	...
	*/
	...
	if (config_chipset & 0x04)        //reset if NTSC mode requested because FPGA boots in PAL mode by default
		OsdReset(1);
	...
	sprintf(s, "CPU clock     : %s MHz", config_chipset & 0x01 ? "28.36": "7.09");
	BootPrint(s);
	sprintf(s, "Blitter speed : %s", config_chipset & 0x02 ? "fast": "normal");
	BootPrint(s);
	...
	while (1)
	{
		...
		if (c1 & CMD_RDTRK)
		{
			...
			{//command phase
				...
				{
					cmd = 0;
					SPI(0xAA); //command header 0xAA55
					SPI(0x55);
...
void User(void)
{
	...
	case MENU_SETTINGS_CHIPSET1 :
		...
		strcpy(s, "      CPU : ");
		strcat(s, config_chipset & 0x01 ? "28.36MHz " : " 7.09MHz");
		OsdWrite(2, s, menusub==0);

		strcpy(s, "  Blitter : ");
		strcat(s, config_chipset & 0x02 ? "fast  " : "normal");
		OsdWrite(3, s, menusub==1);
	...
	case MENU_SETTINGS_DRIVES1 :
		...
		strcpy(s, "   speed    : ");
		strcat(s, config_floppy_speed ? "2x " : "1x");
		OsdWrite(3, s, menusub==1);
