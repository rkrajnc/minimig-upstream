...
;------------------------------------------------------------------------------
	move.w #$9000,$100(A6)  ;bplcon0: hires, 1 bitplane
	move.w #$0000,$102(A6)  ;bplcon1: horizontal scroll = 0
	move.w #$0000,$104(A6)  ;bplcon2
	move.w #$0000,$108(A6)  ;bpl1mod: modulo = 0
	move.w #$0000,$10A(A6)  ;bpl2mod: modulo = 0

	move.w #$003C,$092(A6)	;ddfstrt
	move.w #$00D4,$094(A6)	;ddfstop
	move.w #$2c81,$08e(A6)	;diwstrt
	move.w #$f4c1,$090(A6)	;diwstop
;colours
	move.w #$037f,$180(A6)	; colour0
	move.w #$0fff,$182(A6)	; colour1
...
CopperListCopyLoop:
	...
	move.w	D0,$088(A6) 	;COPJMP1: restart copper

	move.w	#%1000001110010000,$096(A6)	; DMACON
	move.w	#$7FFF,$09E(A6)	; ADKCON
...
;---------------
cmd1:
	cmp.w	#$0001,D0	; print text command?
	bne	no_cmd_1	
...
;---------------
cmd_2:
	cmp.w	#$0002,D0	; memory upload command?
	bne	no_cmd_2
...
;---------------
cmd_3:
	cmp.w	#$0003,D0	; exit bootloader command?
	bne	no_cmd_3
...
; D0
	...
	move.w	#$0002,$09C(A6)	;INTREQ (clear disk block finished irq)
	move.l	#disk_buffer,A0
	move.l	A0,$020(A6)	; DSKPTH
	lsr.w	#1,D0
	ori.w	#$8000,D0	; set DMAEN
	move.w	D0,$024(A6)	; DSKLEN
	move.w	D0,$024(A6)	; DSKLEN (start disk dma)
wait_for_diskdma:
...
title_msg:	
	...
	dc.b	"Bug fixes, mods and extensions by Jakub Bednarski",10,0

bootloader_msg:
	dc.b	10,"Bootloader BYQ080817",10,0
...
incompatible_firmware_msg:
	dc.b	10,"Incompatible PIC firmware!",0	
unknown_command_msg:
